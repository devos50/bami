...
def create_aggregate_result_files(exp_name):
    with open(os.path.join("data", "edge_searches_exp_%s.csv" % exp_name), "w") as out_file:
        out_file.write("peers,nb_size,offline_fraction,malicious_fraction,replication_factor,total_searches,failed_searches\n")
    with open(os.path.join("data", "edge_search_latencies_exp_%s.csv" % exp_name), "w") as out_file:
        out_file.write("peers,nb_size,offline_fraction,malicious_fraction,replication_factor,part,time\n")
